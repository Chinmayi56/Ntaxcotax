...
export default function DataTable({ columns, rows, loading, pageSize = 8, searchable = true, filters, title = "Data", exportable = true, testId = "data-table", searchFields }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      out = out.filter((r) =>
        ...
        (searchFields || []).some((k) => String(r[k] ?? "").toLowerCase().includes(q))
      );
    }
    if (sort.key) {
      out = [...out].sort((a, b) => {
